...
export function addAccount(address, keystring, name, desc, userProfileAddress, publicKey, privateKey) {
    return {
        type: ACC_ACTION.NEW_ACCOUNT_ADDED_PENDING,
        payload: {address, keystring, name, desc, userProfileAddress,publicKey, privateKey}
    }
}
...
export function joiningNextIdWallet(account, hash) {
    return {
        type: ACC_ACTION.JOINING_NEXTID_WALLET,
        payload: { account, hash }
    }
}

export function joinedNextIdWallet(address, contractAddress) {
    return {
        type: ACC_ACTION.JOINED_NEXTID_WALLET,
